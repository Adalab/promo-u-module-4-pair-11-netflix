...
UPDATE `netflix`.`movies` SET `title` = 'Friends', `genre` = 'Comedia' WHERE (`idMovies` = '5');
UPDATE `netflix`.`movies` SET `title` = 'Gambita de dama', `genre` = 'Drama' WHERE (`idMovies` = '4');
INSERT INTO `netflix`.`movies` (`title`, `genre`, `image`, `category`, `year`) VALUES ('Hocus pocus', 'Infantil', 'http://localhost:4000/hocus-pocus.jpeg', 'Top 10', '1993');
